...
public class SaveOutLinkInstantWork extends SaveBatchBaseWork {

	private static Logger logger = LoggerFactory
			.getLogger(SaveOutLinkInstantWork.class);

	public SaveOutLinkInstantWork(String type) {
		super(type);
		saveBatch = 1000;
		maxWait = 10000;
	}
	...
	protected void doBatchSave(Task task) {
		...
		watch.start();
		logger.debug("[SaveInstantOutLinkInBatch] - start. count={}",
		...
		watch.stop();
		logger.debug("[SaveInstantOutLinkInBatch] - cost={} ms | count={}",
				new Object[] { watch.getElapsedTime(), linkDatas.size() });
		watch.start();
		logger.debug("[SaveInstantOutSeedInBatch] - start. count={}",
				new Object[] { seedDatas.size() });
		// 批量保存seed
		SeedDataDAO.getInstance().saveBatch(seedDatas,
				saveStrategy.getSeedDS(), saveStrategy.getSeedDB(),
				saveStrategy.getSeedTable() + "_read", false, true);
		watch.stop();
		logger.debug("[SaveInstantOutSeedInBatch] - cost={} ms | count={}",
